...
class LivestockRegistration(Resource):
    @livestock_ns.response(201, 'Livestock registered successfully')
    @livestock_ns.response(400, 'Bad Request')
    @livestock_ns.doc(description='가축 등록 API')
...
class LivestockDeletion(Resource):
    @livestock_ns.response(200, 'Livestock deleted successfully')
    @livestock_ns.response(500, 'Internal Server Error')
    @livestock_ns.doc(description='가축 삭제 API')
...
def queryLivestockListByUid(uid):
    ...
    if not temp:
        raise Exception('uid does not exist')
    result = []
    for i in temp:
...
class LivestockManagerByUid(Resource):
    @livestock_ns.response(200, 'Livestock list retrieved successfully')
    @livestock_ns.response(404, 'uid does not exist')
    @livestock_ns.doc(description='가축 조회 API(uid)')
    def get(self, uid):
        try:
...
class LivestockManagerByUidAndType(Resource):
    @livestock_ns.response(200, 'Livestock list retrieved successfully')
    @livestock_ns.response(404, 'uid does not exist')
    @livestock_ns.doc(description='가축 조회 API(uid, livestock_stock)')
...
class LivestockManagerLastByUidAndType(Resource):
    @livestock_ns.response(200, 'Livestock retrieved successfully')
    @livestock_ns.response(404, 'uid does not exist')
    @livestock_ns.doc(description='마지막 가축 조회 API')
    def get(self, uid, livestock_type):
        try:
